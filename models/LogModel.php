...
class LogModel
{
	...
	public function getClienteRol($usuario)
	{
		$sql_cr ="select u.usuario as usuario, r.rol  as rol , u.id_usuario from usuarios u inner join roles r on u.id_rol = r.id_rol where usuario ='" .$usuario ."' "     ;
	...
	public function enableAdmin($usuario , $pwd)
	{
		$sql_Admin = "select u.id_usuario from usuarios u where u.usuario ='" .$usuario ."' and u.pwd='" .$pwd. "'" ; 
	
		$enableAdmin = $this->db->prepare($sql_Admin);
		$enableAdmin->execute() ;
		return $enableAdmin->rowCount() ;
	}
	...
	public function InsertCustomer($username, $password, $razonsocial, $cuit, $asesor, $region, $telefono,  $email, $web, $direccion, $altura, $piso, $depto , $localidad, $alto, $ancho, $id_rol, $habilitada, $tipo_plan)
	{
		$dir = null ;
